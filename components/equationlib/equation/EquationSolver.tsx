...
export default function EquationSolver() {
  ...
  return (
    <div className="flex h-full min-w-0 flex-col bg-[var(--surface-1)] text-[#c8d8e8]">
      ...
      <div className="flex h-10 shrink-0 items-end gap-1 border-b border-[var(--border)] bg-[var(--surface-2)] px-3 pt-1">
        {MODES.map((mode) => (
      ...
      <div className="mode-responsive-grid grid min-h-0 flex-1 grid-cols-[minmax(400px,1fr)_minmax(300px,0.62fr)]">
        <div className="flex min-h-0 flex-col border-r border-[var(--border)] p-4">
          ...
          }} className="min-h-40 flex-1 resize-none rounded-md border border-[var(--border-strong)] bg-[var(--input-bg)] p-4 font-mono text-[15px] leading-7 text-white outline-none focus:border-[#4c7ca8]" aria-label="Equation input" spellCheck={false} />
        ...
        <div className="min-h-0 overflow-y-auto p-4">
          ...
          <div className="mt-2 min-h-36 rounded-md border border-[var(--border-strong)] bg-[var(--input-bg)] p-4 font-mono">
            {error ? <div className="text-[12px] text-[#ed7272]">{error}</div> : solutions.length > 0 ? solutions.map((solution) => <div key={solution} className="border-b border-[var(--border)] py-2 text-[17px] font-bold text-white last:border-b-0">{solution}</div>) : <div className="text-[11px] text-[#3d5872]">Enter coefficients or equations, then press SOLVE.</div>}
          </div>
          <div className="mt-3 rounded-md border border-[var(--border)] bg-[var(--surface-2)] p-3 text-[10px] leading-5 text-[#657f98]">
