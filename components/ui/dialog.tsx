...
export function AppDialog({ open, onOpenChange, title, description, icon, children, footer, danger = false }: AppDialogProps) {
  ...
  return createPortal(
    <AnimatePresence>
      {open && (
        <motion.div className="app-dialog-overlay fixed inset-0 z-[350] flex items-center justify-center p-4 backdrop-blur-[2px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onOpenChange(false); }}>
          <motion.div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="app-dialog-title" initial={{ opacity: 0, y: 14, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.98 }} transition={{ duration: 0.18, ease: "easeOut" }} className="app-dialog w-full max-w-[440px] overflow-hidden rounded-md border">
            <div className="flex items-start gap-3 border-b border-[var(--border)] px-5 py-4">
              ...
              <div className="min-w-0 flex-1">
                <h2 id="app-dialog-title" className="text-[14px] font-bold text-[var(--text)]">{title}</h2>
                {description && <p className="mt-1 text-[10px] leading-4 text-[var(--text-muted)]">{description}</p>}
              </div>
            ...
            <div className="flex items-center justify-end gap-2 border-t border-[var(--border)] bg-[var(--surface-2)] px-5 py-3">{footer}</div>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>,
