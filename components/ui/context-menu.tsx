        ...
        <AnimatePresence>
          {position && (
            <motion.div
              ...
              className="app-context-menu fixed z-[250] w-[236px] overflow-hidden rounded-md border p-1.5 backdrop-blur-md"
              style={{ left: position.x, top: position.y, transformOrigin: "top left" }}
            >
              {entries.map((entry, index) => {
                if (entry.type === "separator") return <div key={index} role="separator" className="my-1 h-px bg-[var(--border)]" />;
                if (entry.type === "label") return <div key={index} className="px-2.5 py-1.5 text-[9px] font-bold tracking-[0.18em] text-[var(--text-subtle)]">{entry.label}</div>;
                return (
                  <button
                    ...
                    className={`flex min-h-9 w-full items-center gap-2.5 rounded px-2.5 text-left text-[11px] outline-none transition-colors focus:bg-[var(--surface-hover)] hover:bg-[var(--surface-hover)] disabled:pointer-events-none disabled:opacity-35 ${entry.destructive ? "text-[#d84f5f]" : ""}`}
                  >
